...
function UpdateEncounter() {
	...
	const handleUpdate = (event) => {
		...
		const newEncounter = {
			//set the values for the updated encounter
		...
		if (newEncounter.description.length === 0) {
			//If not updated, set to original values
		...
		const init = {
			//use PUT to update the encounter
			method: "PUT",
			headers: {
	...
	const handleDescriptionChange = (event) => {
		//if the value is changed, use the new value
	...
	return (
		//form for submiting updates to the encounter
