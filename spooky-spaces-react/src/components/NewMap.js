...
export default function NewMap() {
	...
	const handleAdd = (event) => {
		//set values for the new location
	...
	return (
		<>
			...
			<GoogleMap //generate the map
				...
				onLoad={onMapLoad}
			>
				{locations.map((l) => ( //get all the locations and adds markers
					<Marker
						key={l.locationId}
				...
				{markers.map(
					(
						marker //for generating new markers
					) => (
						<Marker
						...
						<div className="text-center">
							...
							<div className="text-center">
								<h4 className="text-primary">
									<u>
										<Link to={`/location/${selected2.locationId}`}>{name}</Link>
									</u>
								</h4>
								<p> {address} </p>
							</div>
